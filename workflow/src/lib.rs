...
use obelisk::{
    ...
    workflow::workflow_support::new_join_set_named,
};
use stargazers::{
    db,
...
impl Guest for Component {
    ...
    fn star_added_parallel(login: String, repo: String) -> Result<(), String> {
        ...
        if description.is_none() {
            ...
            let join_set_info = new_join_set_named(&format!("info_{login}"));
            let join_set_settings = new_join_set_named(&format!("settings_{login}"));
