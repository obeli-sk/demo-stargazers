...
// Package workflowsupport represents the imported interface "obelisk:workflow/workflow-support@2.0.0".
...
// ScheduleAt represents the type alias "obelisk:workflow/workflow-support@2.0.0#schedule-at".
//
// See [time.ScheduleAt] for more information.
type ScheduleAt = time.ScheduleAt

// JoinSetID represents the imported type alias "obelisk:workflow/workflow-support@2.0.0#join-set-id".
//
// See [execution.JoinSetID] for more information.
type JoinSetID = execution.JoinSetID

// DelayID represents the type alias "obelisk:workflow/workflow-support@2.0.0#delay-id".
//
// See [execution.DelayID] for more information.
type DelayID = execution.DelayID

// ExecutionID represents the type alias "obelisk:workflow/workflow-support@2.0.0#execution-id".
//
// See [execution.ExecutionID] for more information.
type ExecutionID = execution.ExecutionID

// ResponseID represents the type alias "obelisk:workflow/workflow-support@2.0.0#response-id".
//
// See [execution.ResponseID] for more information.
type ResponseID = execution.ResponseID

// ClosingStrategy represents the enum "obelisk:workflow/workflow-support@2.0.0#closing-strategy".
//
...
// JoinSetCreateError represents the variant "obelisk:workflow/workflow-support@2.0.0#join-set-create-error".
//
...
// JoinNextError represents the variant "obelisk:workflow/workflow-support@2.0.0#join-next-error".
//
...
//
//go:nosplit
func Sleep(scheduleAt ScheduleAt) {
	scheduleAt0, scheduleAt1, scheduleAt2 := lower_ScheduleAt(scheduleAt)
	wasmimport_Sleep((uint32)(scheduleAt0), (uint64)(scheduleAt1), (uint64)(scheduleAt2))
	return
}
...
//
//go:nosplit
func SubmitDelay(joinSetID JoinSetID, timeout ScheduleAt) (result DelayID) {
	joinSetId0 := cm.Reinterpret[uint32](joinSetID)
	timeout0, timeout1, timeout2 := lower_ScheduleAt(timeout)
	wasmimport_SubmitDelay((uint32)(joinSetId0), (uint32)(timeout0), (uint64)(timeout1), (uint64)(timeout2), &result)
	return
}
...
//
//	new-join-set-named: func(name: string, closing-strategy: closing-strategy) -> result<join-set-id,
//	join-set-create-error>
//
//go:nosplit
func NewJoinSetNamed(name string, closingStrategy ClosingStrategy) (result cm.Result[JoinSetCreateErrorShape, JoinSetID, JoinSetCreateError]) {
...
//
//go:nosplit
func JoinNext(joinSetID JoinSetID) (result cm.Result[ResponseIDShape, ResponseID, JoinNextError]) {
	joinSetId0 := cm.Reinterpret[uint32](joinSetID)
	wasmimport_JoinNext((uint32)(joinSetId0), &result)
	return
}
