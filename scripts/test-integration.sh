...
# Use mock server unless TEST_OPENAI_API_KEY is already set
if [[ -z "${TEST_OPENAI_API_KEY:-}" ]]; then
    echo "TEST_OPENAI_API_KEY not set, using mock OpenAI server"
    ...
    echo "TEST_OPENAI_API_KEY is set, using real OpenAI API"
    # TEST_OPENAI_API_BASE_URL can optionally be set for custom endpoints
fi
